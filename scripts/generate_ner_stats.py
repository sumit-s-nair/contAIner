...
train_data = load_data("datasets/intent-dataset/data/train.jsonl")
val_data = load_data("datasets/intent-dataset/data/validation.jsonl")
test_data = load_data("datasets/intent-dataset/data/test.jsonl")
...
def evaluate_ner(model_dir, dataset):
    ...
    metrics = {
        "n": len(dataset),
        "precision": precision_score(all_true_seqs, all_pred_seqs, average="weighted", zero_division=0),
        "recall": recall_score(all_true_seqs, all_pred_seqs, average="weighted", zero_division=0),
        "f1": f1_score(all_true_seqs, all_pred_seqs, average="weighted", zero_division=0)
    }
...
models_to_run = [
    ("intent_classifier_v1", "outputs/intent_classifier_v1/final_model"),
    ("intent_classifier_current", "outputs/intent_classifier/final_model")
]
...
for name, path in models_to_run:
    ...
    stats[name] = {
        ...
        "split_variation": {
            "train_minus_test_f1": m_train["f1"] - m_test["f1"],
            "train_minus_validation_f1": m_train["f1"] - m_val["f1"],
            "validation_minus_test_f1": m_val["f1"] - m_test["f1"]
        }
    }

out_path = "results/ner_research_stats.json"
